...
- [x] 1.2 Add Factory child-session execution and collection for triage decisions with separate worker authority.
- [x] 1.3 Prove malformed, oversized, secret-bearing, and policy-shaped decisions are refused.

## 2. Factory SDLC operation

- [x] 2.1 Add the typed `issue-sdlc` manifest operation and validate its triage, implementation, acceptance, and delivery declarations.
- [x] 2.2 Stop before implementation on `needs_input` or `refused` and return canonical evidence.
- [x] 2.3 Feed validated ready context into existing isolated software-change implementation and coordinator-owned acceptance.
- [x] 2.4 Produce independently verifiable sanitized failure questions only for recoverable implementation/check failures.
- [x] 2.5 Preserve integrity/authority failures without any delivery request.
...
- [x] 3.4 Verify and post idempotent question/failure comments; launch a new ephemeral attempt after one authorized answer.
- [x] 3.5 Keep stable branch identity while giving each attempt a stable unique run identity.

## 4. Delivery and verification

- [x] 4.1 Retain moving-base, patch/result digest, secret-scan, and independent acceptance checks before draft delivery.
- [x] 4.2 Mark success verified-for-review without submitting a GitHub approval or merge.
- [x] 4.3 Clean successfully collected question and delivery attempts; preserve bounded forensic failures.
