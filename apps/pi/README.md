...
The **pi** harness as a portable Barista app: an [App Manifest](manifest.json)
plus a semantic-transfer adapter that runs against any conformant Host API
provider.

- **Adapter:** `barista_app_pi` — implements the SDK `Adapter` interface
...
`Dockerfile` packages Pi 0.73.1 on a digest-pinned multi-architecture Node
base. It includes CA certificates, Git, and ripgrep, runs as the non-root `node`
user, and keeps the session alive with a fixed inert entrypoint. Pi invocations
run through Host API `Exec`; model credentials come only from the manifest's
provider-resolved secret reference.

Build from the repository root and always publish both supported platforms:

```sh
docker buildx build -f apps/pi/Dockerfile \
...
```bash
cd apps/pi && uv run --extra test pytest -q
```
