...
function renderFeatures(p){const root=$('features');clear(root);const fs=p?.features||[];$('feature-count').textContent=`${fs.length} ${fs.length===1?'feature':'features'}`;if(!fs.length){root.append(el('li','empty','The verified plan will appear after BRD approval.'));return}fs.forEach((f,i)=>{const li=el('li','feature');li.append(el('span','ordinal',String(i+1)));const body=el('div');body.append(el('h3','',f.title));body.append(el('p','',f.summary));body.append(el('div','deps',f.dependencies.length?`Waits for: ${f.dependencies.join(', ')}`:'No dependencies'));li.append(body);const badge=el('span','state',f.status.replaceAll('_',' '));badge.dataset.state=f.status;li.append(badge);root.append(li)})}
