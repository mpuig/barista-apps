...
function pickProgram(){if(selected){const found=state.programs.find(p=>p.program_id===selected);if(found)return found}return state.current_program||state.programs[0]||null}
...
function pair(term,value,mono=false){const d=el('div');d.append(el('dt','',term));d.append(el('dd',mono?'mono':'',value||'—'));return d}function renderEvidence(p){const d=$('evidence');clear(d);if(!p){d.append(pair('Controller','Healthy'));d.append(pair('Cleanup','No active scenario'));return}d.append(pair('Program',p.program_id,true));d.append(pair('Root issue',`#${p.issue_number}`));d.append(pair('BRD digest',p.brd.digest,true));d.append(pair('Plan digest',p.plan_digest,true));d.append(pair('Accepted',p.acceptance?.assembled_commit,true));const dep=p.deployment;d.append(pair('Deployment',dep?.state));d.append(pair('Image',dep?.result?.image_digest,true));d.append(pair('Cleanup',p.terminal?'Terminal; reset available':'Preserving active work'))}
